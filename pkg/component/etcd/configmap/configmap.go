...
func (c *component) syncConfigmap(ctx context.Context, cm *corev1.ConfigMap) error {
    ...
    advertise-client-urls: ` + protocol + `://0.0.0.0:` + clientPort + `
    ...
    # The URLs needed to be a comma-separated list.
    initial-advertise-peer-urls: ` + c.values.PeerServiceName + `@` + c.values.EtcdNameSpace + `@` + serverPort + `

    # Initial cluster token for the etcd cluster during bootstrap.
    initial-cluster-token: initial
...
func getObjectMeta(val Values) metav1.ObjectMeta {
	...
	return metav1.ObjectMeta{
		Name:      fmt.Sprintf("etcd-bootstrap-%s", string(val.EtcdUID[:6])),
